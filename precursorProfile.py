...
def precursorProfile(casename: str, width: int, starttime=None, offset=None,
    ...
    if starttime_mode:
        endtime = starttime + width # type: ignore
    ...
    for readfile in readfiles:
        ...
        if first_readfile:
            ...
            if starttime_mode:
                if (starttime < lower_time_limit) or (endtime > upper_time_limit): # type: ignore
                ...
                N_windows = int( (upper_time_limit - lower_time_limit) // offset ) # type: ignore
        ...
        if quantity in ['Rf','Ri']:
            data_to_write = heights[:-1]
        else:
            data_to_write = np.array(heights)

        for i in range(N_windows):
            if not starttime_mode:
                starttime = lower_time_limit + offset*i # type: ignore
...
if __name__ == '__main__':
    ...
    conflicts.add_argument("-r", "--restart",
